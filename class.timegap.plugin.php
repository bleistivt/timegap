...
class TimeGapPlugin extends Gdn_Plugin {
    ...
    public function discussionController_beforeCommentDisplay_handler($sender, $args) {
        ...
        if ($gap < Gdn::config('TimeGap.MinDays', 7) - 1) {
            return;
        } elseif ($gap < 30) {
            $gap = sprintf(Gdn::translate('%s days later'), (int)$gap);
            $class = 'Days';
        } elseif ($gap < 60) {
            $gap = Gdn::translate('1 month later');
            $class = 'Month';
        } elseif ($gap < 365) {
            $gap = sprintf(Gdn::translate('%s months later'), (int)($gap / 30));
            $class = 'Months';
        } elseif ($gap < 720) {
            $gap = Gdn::translate('1 year later');
            $class = 'Year';
        } else {
            $gap = sprintf(Gdn::translate('%s years later'),  (int)($gap / 365));
            $class = 'Years';
        }
